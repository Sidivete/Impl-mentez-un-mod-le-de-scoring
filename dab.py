...
FLASK_URL = "http://13.49.238.55:5600/"
...
def load_data():
    ...
    return data
data = load_data()
data.rename(columns={'OCCUPATION_TYPE_Waiters\/barmen staff': 'OCCUPATION_TYPE_Waiters_barmen_staff'},inplace=True)
...
url_predict_client = FLASK_URL + "predict/" + str(int(idx))
print("idx : ", url_predict_client)
...
ID_to_predict.rename(columns={'OCCUPATION_TYPE_Waiters\/barmen staff': 'OCCUPATION_TYPE_Waiters_barmen_staff'},inplace=True)
print("ID_to_predict", ID_to_predict["OCCUPATION_TYPE_Waiters_barmen_staff"])
...
def customer_details():
    ...
    for i, c in enumerate(sel,1):
        ...
        if ((i == 1) | (i == 2)):
            ...
            column = 1
        o=c
        if c == "OCCUPATION_TYPE_Waiters/barmen staff":
            tmp=ID_to_predict['OCCUPATION_TYPE_Waiters_barmen_staff'].iloc[0]
            o='OCCUPATION_TYPE_Waiters_barmen_staff'
        else:
            tmp=ID_to_predict[c].iloc[0]
        fig1.add_trace(go.Indicator(
            mode = "number+delta",
            value = tmp,
            delta = {'reference': np.mean(data[o]),
                    'valueformat': '.0f',
